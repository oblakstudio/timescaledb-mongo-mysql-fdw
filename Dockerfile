...
RUN PGBOUNCER_EXPORTER_VERSION="0.7.0"&& \
  ARCH=$(dpkg --print-architecture) && \
  URL=https://github.com/prometheus-community/pgbouncer_exporter/releases/download/v$PGBOUNCER_EXPORTER_VERSION/pgbouncer_exporter-$PGBOUNCER_EXPORTER_VERSION.linux-$ARCH.tar.gz && \
  echo $URL
...
RUN PGBOUNCER_EXPORTER_VERSION="0.7.0" && \
  ...
  $URL && \
  cd /tmp && \
  tar xvzf /tmp/pkg.tgz "pgbouncer_exporter-${PGBOUNCER_EXPORTER_VERSION}.linux-$ARCH/pgbouncer_exporter" && \
  mv -v "/tmp/pgbouncer_exporter-${PGBOUNCER_EXPORTER_VERSION}.linux-$ARCH/pgbouncer_exporter" /usr/local/bin/pgbouncer_exporter && \
  rm -rfv /tmp/pkg.tgz "/tmp/pgbouncer_exporter-${PGBOUNCER_EXPORTER_VERSION}.linux-${ARCH}"
...
RUN curl -1sLf 'https://downloads.enterprisedb.com/WWyW67XPeT8AYO2pfRepCRogiYvOwHrz/enterprise/setup.deb.sh' | bash
RUN apt-get update
RUN apt-get -y install postgresql-15-mongo-fdw postgresql-15-mysql-fdw
